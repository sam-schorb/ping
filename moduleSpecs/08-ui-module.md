...
- **RoutingResult**: use `RouteResult` + `RouteData` from `moduleSpecs/04-routing-delay.md`.
  - UI consumes `edgeRoutes: Map<edgeId, RouteData>` with `{ points, svgPathD, totalLength }`.
- **PaletteItem**: use the `PaletteItem` shape from `moduleSpecs/01-node-registry.md` (returned by `buildPalette()`).
...
1. **Build `GroupDefinition`** (see `moduleSpecs/01-node-registry.md` / `moduleSpecs/03-serialisation.md`):
...
- Group port ordering uses the **custom layout rules** from `moduleSpecs/01-node-registry.md`.
...
- **Control inputs**: add one‑by‑one via dropdown from **unconnected internal control inputs**.
...
- A single internal control input **cannot** map to multiple group controls.
- An internal control input that is already driven by an internal cable is **not selectable by default** as a group control.
- The dialog may offer an explicit `Expose Instead…` action for such blocked control inputs.
  - If used, it must warn the user that the internal control cable will be disconnected and replaced by a group-boundary control mapping.
- Unmapped internal control inputs remain unexposed.
- If a targeted node also has a stored param, that stored param keeps its default value until a live control pulse arrives.
